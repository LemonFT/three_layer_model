...
public class StudentgradeBLL {
    ...
    public List<Studentgrade> searchStudentGrade(String searchField, int sortOption, int filter) {
        ...
        if (columnFilter.equals("")) {
            sql += "where EnrollmentID like '%" + searchField + "%' or "
                    + " Concat(course.CourseID, ' - ', course.Title) like '%" + searchField + "%' or "
                    + " Concat(person.PersonID, ' - ', person.Lastname, person.Firstname) like '%" + searchField
                    + "%' or "
    ...
    public List<Course> searchCourse(String keySearch) {
        String sql = "select * from course where Concat(CourseID, ' - ', Title) like '%" + keySearch + "%'"
                + " OR Title like '%" + keySearch + "%'";
        try {
    ...
    public List<Person> searchStudent(String keySearch) {
        String sql = "select * from person where HireDate is null and (Concat(PersonID, ' - ', Lastname, Firstname) like '%"
                + keySearch + "%'" + " OR CONCAT(Lastname, ' ', Firstname) like '%" + keySearch + "%')";
        try {
    ...
    public String insertStudentGrade(String courseID, String studentID, String result) {
        ...
        try {
            boolean rs_insertStudentGrade = StudentgradeDAL
                    .insertStudentGrade(new Studentgrade(courseID_Int, studentID_Int, result_Double));
    ...
    public String updateStudentGrade(String courseIDnow, String studentIDnow, int enrollmentID, String courseID,
        ...
        try {
            boolean rs_updateStudentGrade = StudentgradeDAL
                    .updateStudentGrade(new Studentgrade(enrollmentID, courseID_Int, studentID_Int, result_Double));
